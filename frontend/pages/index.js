...
export default function Home(){
  ...
  useEffect(()=>{ 
    setShades([])
    if(API && serviceId) fetch(`${API}/api/shades?serviceId=${serviceId}`).then(r=>r.json()).then(setShades) 
  },[serviceId])
      ...
      <form onSubmit={submit}>
        ...
        {serviceId && (
          <label> Tint Shade
            ...
            <div className="helper">Enable/disable shades in Admin Settings (no redeploy needed).</div>
          </label>
        )}
        ...
        {msg && <div style={{ color: ok ? '#7CFC00' : '#ff6b6b' }}>{msg}</div>}
