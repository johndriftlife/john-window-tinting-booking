...
export default function Home(){
  ...
  useEffect(()=>{
    if(!API) return
    fetch(`${API}/api/services`).then(r=>r.json()).then(setServices).catch(()=>setServices([]))
  },[])

  useEffect(()=>{
    setShades([])
    if(!API || !serviceId) return
    fetch(`${API}/api/shades?serviceId=${serviceId}`).then(r=>r.json()).then(setShades).catch(()=>setShades([]))
  }, [serviceId])
  ...
  async function submit(e){
    ...
    if(!API){ setMsg('Set NEXT_PUBLIC_API_BASE_URL on the frontend.'); return }
    try{
      ...
      // reset time only so users can book another
      setTime('')
