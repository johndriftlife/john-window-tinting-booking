...
export default function Home(){
  ...
  const [windowArea, setWindowArea] = useState('Front doors')

  const [date, setDate] = useState('')
  const [time, setTime] = useState('')
  const [times, setTimes] = useState([])

  const [deposit, setDeposit] = useState({ amountCents: 5000, currency: 'usd' })
  ...
  useEffect(()=>{
    if(API) fetch(`${API}/api/deposit/config`).then(r=>r.json()).then(setDeposit).catch(()=>{})
  },[])
  ...
  function fmtAmount(cents, cur){
    ...
    return `${cur?.toUpperCase()==='USD'?'$':''}${amt} ${cur?.toUpperCase()}`.trim()
  }
  ...
  async function submit(e){
    ...
    try{
      const res = await fetch(`${API}/api/deposit/session`, {
        ...
        body: JSON.stringify({
          name, phone, brand, model, windowArea,
    ...
    <div className="container">
      ...
      <form onSubmit={submit}>
        ...
        <label> Window to work on
          <select value={windowArea} onChange={e=>setWindowArea(e.target.value)}>
        ...
        <label> Deposit
          <input className="readonly" value={fmtAmount(deposit.amountCents, deposit.currency)} readOnly />
        </label>
