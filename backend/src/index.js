...
// full list (for out-of-stock rendering)
...
app.post('/api/book', async (req,res) => {
  try {
    ...
    if (!windowArea) return res.status(400).json({ error: 'windowArea required' })
    const booking = await prisma.booking.create({
      data: { name, phone, brand: brand||null, model: model||null, serviceId, shades: shades.join(','), windowArea, date, time }
    })
